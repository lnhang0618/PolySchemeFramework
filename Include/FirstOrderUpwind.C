...
void FirstOrderUpwind::initializeU(SchemesData& data) {
...
void FirstOrderUpwind::iterateU(SchemesData& data) {
    ...
    for(; data.T < data.T_end; data.T += data.dt) {
        ...
        data.u_new[0] = data.u[0] 
                        - data.c * data.dt / data.dx 
                        * (data.u[0] - data.u[data.N - 1]);
        data.u_new[data.N - 1] = data.u[data.N - 1] 
                                - data.c * data.dt / data.dx 
                                * (data.u[data.N - 1] - data.u[data.N - 2]);
